...
def make_hook(activation_list):
    ...
    def hook_fn(module, input, output):
        ...
        # Debug print: show the shape of the captured activation.
        print(f"[DEBUG] Captured activation with shape: {act.shape}")
...
def process_and_save_activations(model, layer_module, dataloader, save_prefix, chunk_size, layer_idx):
    ...
    for i, batch in enumerate(tqdm(dataloader, desc=f"Processing {save_prefix} Model")):
        ...
        if (i + 1) % chunk_size == 0:
            if activation_list:
                ...
                print(f"[INFO] Saved chunk {chunk_idx} to {filename}")
                chunk_idx += 1
    ...
    if activation_list:
        ...
        torch.save(chunk_tensor, filename)
        file_list.append(filename)
        print(f"[INFO] Saved final chunk {chunk_idx} to {filename}")
...
chunk_size = 500  # Adjust based on available memory (flush every 500 batches)
...
# Print the SAE configuration for debugging.
print("[DEBUG] SAE configuration:")
print(sae.cfg)
...
for base_file, finetuned_file in zip(base_file_list, finetuned_file_list):
    ...
    # Print shapes of the loaded chunks for debugging.
    print(f"[DEBUG] Loaded base_chunk from {base_file} with shape: {base_chunk.shape}")
    print(f"[DEBUG] Loaded finetuned_chunk from {finetuned_file} with shape: {finetuned_chunk.shape}")

    # Diagnostic print before encoding.
    print(f"[DEBUG] Sending base_chunk and finetuned_chunk to SAE encoder with shapes {base_chunk.shape} and {finetuned_chunk.shape} respectively.")
    ...
    print(f"[INFO] Saved sparse codes for chunk from {base_file} and {finetuned_file} to {base_sc_filename} and {finetuned_sc_filename}.")
    
    # Compute metrics for this chunk.
    l1_diff = torch.abs(base_sparse - finetuned_sparse)
    chunk_l1_mean = l1_diff.mean().item()
    print(f"[INFO] Chunk L1 difference mean: {chunk_l1_mean}")
    ...
    print(f"[INFO] Chunk cosine similarity mean: {chunk_cos_mean}")
